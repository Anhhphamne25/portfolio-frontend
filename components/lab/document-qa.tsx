...
export function DocumentQA() {
  ...
  const handleAsk = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/doc-qa", { method: "POST", body: formData })
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      >
        ...
        />
        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${file ? "bg-primary/20" : "bg-card border border-border"}`}>
          <svg className={`w-6 h-6 ${file ? "text-primary" : "text-muted-foreground"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      ...
      {/* Chat interface */}
      <div className={`rounded-2xl border border-border overflow-hidden transition-all ${!file ? "opacity-50 pointer-events-none" : ""}`}>
        <div className="bg-secondary px-4 py-3 border-b border-border flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-primary" />
          <p className="text-xs font-semibold text-foreground">Document Q&amp;A — RAG Mode</p>
          {file && <span className="ml-auto text-xs text-muted-foreground truncate max-w-32">{file.name}</span>}
        </div>
        ...
        <div className="h-64 overflow-y-auto p-4 flex flex-col gap-3 bg-card">
          {messages.length === 0 && (
            <div className="h-full flex items-center justify-center">
              <p className="text-xs text-muted-foreground text-center">
                {file ? "Upload complete. Ask your first question below." : "Upload a PDF to get started."}
              </p>
            </div>
          )}
          {messages.map((msg, i) => (
            <div key={i} className={`flex gap-2 items-start ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
              <div className={`w-7 h-7 rounded-xl flex items-center justify-center text-xs font-bold shrink-0 ${msg.role === "user" ? "bg-primary text-primary-foreground" : "bg-accent text-foreground"}`}>
                {msg.role === "user" ? "U" : "AI"}
              </div>
              <div className={`px-3 py-2 rounded-xl text-sm leading-relaxed max-w-xs md:max-w-sm ${msg.role === "user" ? "bg-primary text-primary-foreground rounded-tr-none" : "bg-secondary text-foreground rounded-tl-none"}`}>
                {msg.text}
              </div>
            </div>
          ))}
          {loading && (
            <div className="flex gap-2 items-start">
              <div className="w-7 h-7 rounded-xl bg-accent flex items-center justify-center text-xs font-bold text-foreground shrink-0">AI</div>
