...
const TOPICS = ["Array", "Dynamic Programming", "Graph", "Tree", "String", "Hash Map", "Binary Search", "Stack"]
      ...
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <div className="flex flex-col gap-2">
          ...
          <select
            ...
            className="px-3 py-2 rounded-xl bg-secondary border border-border text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-accent"
          >
            {LANGUAGES.map((l) => (
      ...
      {result && (
        <div className="flex flex-col gap-4 rounded-2xl border border-border bg-card p-5 animate-in fade-in slide-in-from-bottom-2 duration-300">
          <div className="flex items-center gap-3">
            ...
            <span className="px-2.5 py-1 rounded-full bg-primary/15 text-primary text-xs font-semibold">{difficulty}</span>
            <span className="px-2.5 py-1 rounded-full bg-secondary text-muted-foreground text-xs">{topic}</span>
          </div>
          ...
          {result.examples.length > 0 && (
            <div className="flex flex-col gap-2">
              ...
              {result.examples.map((ex, i) => (
                <div key={i} className="rounded-xl bg-secondary border border-border p-3 text-xs font-mono">
